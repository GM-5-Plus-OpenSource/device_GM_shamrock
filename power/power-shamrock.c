...
//
// This kernel exposes both:
//   /sys/devices/system/cpu/cpu0/core_ctl  -> LITTLE cluster cpu0-cpu3
//   /sys/devices/system/cpu/cpu4/core_ctl  -> BIG cluster cpu4-cpu7
//
// Direct cpuX/online hotplug is unreliable here, so PowerHAL changes core_ctl
// policy only.
//
// Modes:
// - PERF/input/launch/high load: LITTLE 4 + BIG 4
// - IDLE: LITTLE 2 + BIG 1
// - SCREEN_OFF: LITTLE 1 + BIG 0
// - LOW_LOAD: LITTLE 2 + BIG 0
// -----------------------------------------------------------------------------
...
// Some frameworks/legacy PowerHAL glue do not call set_interactive_override().
// This monitor reads common framebuffer/backlight state nodes and applies the
// same dual core_ctl policy when the panel turns off/on.
static int read_screen_on_state(int* on) {
    ...
    // backlight bl_power: 0 = on, non-zero = off/suspend.
    ...
    // Last fallback: brightness 0 usually means panel off. This is less perfect
    // because manual brightness can also be low, so only use it if no state node exists.
    ...
    while (1) {
        ...
        if (read_screen_on_state(&on) == 0) {
            if (last_on == -1) {
                ...
                // Important: after SCREEN_OFF some late framework/vendor hint may put
                // core_ctl back to PERF. While the panel is off, periodically force
                // screen-off policy again so it cannot stay at 4/4.
                off_reapply_ticks++;
